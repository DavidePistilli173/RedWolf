...
namespace rw::time
{
    ...
    */
   template<typename T>
   concept IsChronoClock =
      std::is_same<T, std::chrono::system_clock>::value || std::is_same<T, std::chrono::steady_clock>::value ||
    ...
    */
   template<typename T>
   concept IsConvertibleToChronoDuration = requires(T t) { std::chrono::microseconds(t); };
} // namespace rw::time
